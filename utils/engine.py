...
def train(model: torch.nn.Module, 
    ...
    for epoch in tqdm(range(epochs), colour="green"):
        ...
        if writer:
            ...
            #log model parameters (might disable it later)
            # for name, param in model.named_parameters():
            #     writer.add_histogram(tag=f"Weights/{name}", values=param, global_step=epoch)
            #     if param.grad is not None:
            #         writer.add_histogram(tag=f"Gradients/{name}",values=param.grad, global_step=epoch)
